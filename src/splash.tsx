...
function useBackgroundSync() {
    ...
    const [enabled, setEnabled] = useState(() => localStorage.getItem('splashBackground') === 'true');
    ...
    }, [resolvedBgImage, enabled, bgOpacity, bgBlur, bgBrightness, bgContrast, bgSaturation, bgSize, bgPosition, bgRotation]);
}
    ...
    <Button
      ...
      onClick={() => {
        console.debug('LaunchRoblox clicked');
         ...
         invoke('launch_roblox', { flagsJson: flags, skyboxPath: skybox }).catch(() => console.error('launch_roblox failed'));
      }}
    >
      <span style={{ fontSize: '0.9rem', fontWeight: 600, letterSpacing: '0.5px' }}>Launch Roblox</span>
    </Button>
  );
}
